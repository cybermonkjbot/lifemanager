...
export const DEFAULT_APP_CONFIG: AppConfig = {
  ...
  memeThreadCooldownMs: 150 * 60 * 1000,
  ...
  aiAckRoutingEnabled: true,
  aiTemperature: 0.7,
  aiMaxOutputTokens: 140,
  aiMaxReplyChars: 320,
  aiHistoryLineLimit: 14,
  ...
  humanDelayMaxMs: 80_000,
  humanTypingMinMs: 3_000,
  humanTypingMaxMs: 12_000,
  outboxClaimLimit: 8,
  outboxPollMs: 2_200,
  inboundMergeWindowMs: 35_000,
  manualInterventionCooldownMs: 90 * 1000,
  ...
  romanticMorningCollisionCooldownHours: 6,
...
export async function getConfig(ctx: QueryCtx | MutationCtx, tenantId?: Id<"tenantAccounts">): Promise<AppConfig> {
  ...
  return {
    ...
    aiMaxOutputTokens: Math.round(clamp(parseNumber(map.get("aiMaxOutputTokens"), DEFAULT_APP_CONFIG.aiMaxOutputTokens), 40, 2000)),
    aiMaxReplyChars: Math.round(clamp(parseNumber(map.get("aiMaxReplyChars"), DEFAULT_APP_CONFIG.aiMaxReplyChars), 60, 2400)),
    aiHistoryLineLimit: Math.round(clamp(parseNumber(map.get("aiHistoryLineLimit"), DEFAULT_APP_CONFIG.aiHistoryLineLimit), 4, 120)),
